...
%PYTHON_EXEC% -c "from pathlib import Path; p = Path('.env'); lines = p.read_text(encoding='utf-8', errors='ignore').splitlines() if p.exists() else []; cfg = dict(l.strip().split('=', 1) for l in lines if '=' in l and not l.strip().startswith('#')); tok = cfg.get('DISCORD_BOT_TOKEN', ''); tok_st = '[OK] Token configurado' if (tok and tok != 'seu_discord_bot_token_aqui') else '[AVISO] Token não configurado'; prov = cfg.get('AI_PROVIDER', 'gemini').lower(); g_key = cfg.get('GEMINI_API_KEY', ''); ds_key = cfg.get('DEEPSEEK_API_KEY', ''); c_key = cfg.get('ANTHROPIC_API_KEY', ''); ai_st = ('[OK] Chave detectada (' + cfg.get('GEMINI_MODEL', 'gemini-2.5-flash') + ')') if (prov == 'gemini' and g_key) else (('[OK] DeepSeek detectado (' + cfg.get('DEEPSEEK_MODEL', 'deepseek-chat') + ')') if (prov == 'deepseek' and ds_key) else (('[OK] Claude detectado (' + cfg.get('ANTHROPIC_MODEL', 'claude-3-5-sonnet') + ')') if (prov == 'claude' and c_key) else ('[OK] Chave Gemini detectada' if g_key else '[AVISO] Chave de IA não configurada'))); c_mock = cfg.get('CANVAS_MOCK', 'true').lower() in ('true', '1') or cfg.get('CANVAS_MOCK_MODE', 'true').lower() in ('true', '1'); c_tok = cfg.get('CANVAS_API_TOKEN', ''); c_url = cfg.get('CANVAS_BASE_URL', 'https://puc-rio.instructure.com'); inst = 'PUC-Rio' if 'puc-rio' in c_url.lower() else c_url; lms_prov = cfg.get('LMS_PROVIDER', 'multi').lower(); c_active = lms_prov in ('canvas', 'multi') or c_mock or (c_tok and c_tok != 'seu_canvas_token_aqui'); c_st = ('ATIVADO (' + inst + ') [Modo Mock: ' + ('Ativado' if c_mock else 'Desativado') + ']') if c_active else 'DESATIVADO'; m_url = cfg.get('MOODLE_BASE_URL', 'https://virtual.ufmg.br'); m_user = cfg.get('MOODLE_USERNAME', ''); m_active = lms_prov in ('moodle', 'multi') or (m_user and m_user != 'seu_usuario_moodle') or Path('storage/cookies/session.json').exists(); m_inst = 'UFMG' if 'ufmg' in m_url.lower() else m_url; m_st = ('ATIVADO (' + m_inst + ')') if m_active else 'DESATIVADO'; ai_label = 'IA Gemini:  ' if prov == 'gemini' else ('IA DeepSeek:' if prov == 'deepseek' else ('IA Claude:  ' if prov == 'claude' else 'IA / Motor: ')); print('=================================================================='); print('[STATUS DE INICIALIZAÇÃO]'); print(' • Discord:     ' + tok_st); print(' • ' + ai_label + ' ' + ai_st); print(' • Canvas LMS:  ' + c_st); print(' • Moodle:      ' + m_st); print('==================================================================')"
